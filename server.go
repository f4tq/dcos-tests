...
const (
	...
	DefaultKeepAliveMaxRequests = 10
)
...
func init() {
	...
	flag.IntVar(&keepAliveSeconds, "keep-alive-secs", DefaultKeepAliveSeconds, "KeepAlive header timeout.  Default: 60")
	flag.IntVar(&keepAliveMaxRequests, "keep-alive-max", DefaultKeepAliveMaxRequests, "KeepAlive header max requests.  Default: 10")
	flag.BoolVar(&closeConnection, "close-connection", false, "Inject 'Connection: close'.  Negates keep-alive")
		...
		if period > 0 {
			...
			if s.closeConnection {
				...
				if s.keepAliveSeconds != 0 {
					w.Header().Set("Keep-Alive", fmt.Sprintf("timeout=%d, max=%d", s.keepAliveSeconds, s.keepAliveMaxRequests))
				}
			}
